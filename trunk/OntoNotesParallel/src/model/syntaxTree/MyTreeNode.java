...
public class MyTreeNode {
	...
	public MyTreeNode getLastLeaf() {
		return this.getLeaves().get(this.getLeaves().size()-1);
	}
	...
	public static void main(String args[]) {
		String treeStr = "(ROOT" + "  (IP" + "    (VP (VV 请)" + "      (IP"
				+ "        (VP (VV 听)" + "          (NP" + "            (DNP"
				+ "              (NP (NN 记者) (NN 宫能惠))"
				+ "              (DEG 的))" + "            (NP (NN 报导))))))"
				+ "    (PU 。)))";
		...
		System.out.println(tree.root.getTreeBankStyle(true));

		MyTreeNode copyNode = tree.root.copy();
		System.out.println(copyNode.getTreeBankStyle(true));
	}
	...
	public String toString() {
		...
		while (frontiers.size() > 0) {
			...
			if (tn.children.size() == 0
					&& !tn.parent.value.equalsIgnoreCase("-none-")) {
	...
	private void appendTreeNodeValue(MyTreeNode tn, StringBuilder sb,
			boolean leaf) {
		...
		if (!haveMarkedChild) {
			if (tn.mark) {
				if (tn.value.equalsIgnoreCase("-none-")) {
					...
					// leaf
					if (leaf && tn.children.size() == 1
							&& tn.children.get(0).children.size() == 0) {
						sb.append(" (").append(tn.value).append(" ")
								.append(tn.children.get(0).value).append(")");
					} else {
						sb.append(" (").append(tn.value).append(" )");
					}
				} else {
	...
	public int hashCode() {
		ArrayList<MyTreeNode> leaves = this.getLeaves();
		String str = leaves.get(0).leafIdx + "_"
				+ leaves.get(leaves.size() - 1).leafIdx + "_" + this.toString();
		return str.hashCode();
	}
	...
	public boolean equals(Object em2) {
		...
		if (this.toString().equals(((MyTreeNode) em2).toString())
				&& leaves1.get(0).leafIdx == leaves2.get(0).leafIdx
			...
			if (travelWay[0].equals("left")) {
				for (int i = 1; i < travelWay.length; i++) {
					for (int j = 0; j < this.children.size(); j++) {
						if (this.children.get(j).value
								.equalsIgnoreCase(travelWay[i])) {
	...
	private static void loadArabicRules() {
		...
		headRules.put("NX", new String[][] { { "left", "DT", "DTNN", "DTNNS",
				"DTNNP", "DTNNPS", "DTJJ", "DTNOUN_QUANT", "NOUN_QUANT" } });
		headRules.put("ADJP", new String[][] {
				{ "rightdis", "ADJ", "DTJJ", "ADJ_NUM", "DTADJ_NUM", "JJR",
						"DTJJR" },
				{ "right", "ADJP", "VN", "NOUN", "NNP", "NOFUNC", "NO_FUNC",
						"NNPS", "NNS", "DTNN", "DTNNS", "DTNNP", "DTNNPS",
						"DTJJ", "DTNOUN_QUANT", "NOUN_QUANT" },
				{ "right", "RB", "CD", "DTRB", "DTCD" }, { "right", "DT" } }); // sometimes
																				// right,
																				// sometimes
																				// left
																				// headed??
		headRules.put("ADVP", new String[][] {
				...
				{ "left", "CD", "RP", "NOUN", "CC", "ADJ", "DTJJ", "ADJ_NUM",
						"DTADJ_NUM", "IN", "NP", "NNP", "NOFUNC", "DTRP",
						"DTNN", "DTNNP", "DTNNPS", "DTNNS", "DTJJ",
						"DTNOUN_QUANT", "NOUN_QUANT" } }); // NNP is a gerund
															// that they called
															// an unknown (=NNP,
															// believe it or
															// not...)
		headRules.put("CONJP", new String[][] { { "right", "IN", "RB", "NOUN",
				"NNS", "NNP", "NNPS", "DTRB", "DTNN", "DTNNS", "DTNNP",
				"DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT" } });
		headRules.put("FRAG", new String[][] {
				{ "left", "NOUN", "NNPS", "NNP", "NNS", "DTNN", "DTNNS",
						"DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT" },
				{ "left", "VBP" } });
		headRules
				.put("INTJ", new String[][] { { "left", "RP", "UH", "DTRP" } });
		headRules.put("LST", new String[][] { { "left" } });
		headRules.put("NAC",
				new String[][] {
						{ "left", "NP", "SBAR", "PP", "MWP", "ADJP", "S",
								"PRT", "UCP" }, { "left", "ADVP" } }); // note:
																		// maybe
																		// CC,
																		// RB
																		// should
		...
		headRules.put("NP",
				new String[][] {
						{ "left", "NOUN", "DET+NN", "NNS", "NNP", "NNPS", "NP",
								"PRP", "WHNP", "QP", "WP", "DTNNS", "DTNNPS",
								"DTNNP", "NOFUNC", "NO_FUNC", "DTNOUN_QUANT",
								"NOUN_QUANT" },
						{ "left", "ADJ", "DTJJ", "JJR", "DTJJR", "ADJ_NUM",
								"DTADJ_NUM" }, { "right", "CD", "DTCD" },
						{ "left", "PRP$" }, { "right", "DT" } }); // should the
																	// JJ rule
																	// be left
																	// or right?
		headRules.put("PP", new String[][] {
				{ "left", "PREP", "PP", "MWP", "PRT", "X" },
		...
		headRules.put("PRN", new String[][] { { "left", "NP" } }); // don't get
																	// PUNC
		headRules.put("PRT", new String[][] { { "left", "RP", "PRT", "IN",
				"DTRP" } });
		headRules.put("QP", new String[][] { { "right", "CD", "DTCD", "NOUN",
				"ADJ", "NNS", "NNP", "NNPS", "DTNN", "DTNNS", "DTNNP",
				"DTNNPS", "DTJJ", "DTNOUN_QUANT", "NOUN_QUANT" } });
		...
		headRules.put("S", new String[][] { { "left", "VP", "MWV", "S" },
				{ "right", "PP", "MWP", "ADVP", "SBAR", "UCP", "ADJP" } }); // really
																			...
																			// here!
		headRules.put("SQ",
				new String[][] { { "left", "VP", "MWV", "PP", "MWP" } }); // to
																			// be
																			// principled,
																			// we
																			// need
		...
		headRules.put("SBAR",
				new String[][] {
						{ "left", "WHNP", "WHADVP", "WRB", "RP", "IN", "SBAR",
								"CC", "WP", "WHPP", "ADVP", "PRT", "RB", "X",
								"DTRB", "DTRP" },
						{ "left", "NOUN", "NNP", "NNS", "NNPS", "DTNN",
								"DTNNS", "DTNNP", "DTNNPS", "DTNOUN_QUANT",
								"NOUN_QUANT" }, { "left", "S" } });
		headRules.put("SBARQ", new String[][] {
				{ "left", "WHNP", "WHADVP", "RP", "IN", "SBAR", "CC", "WP",
						"WHPP", "ADVP", "PRT", "RB", "X" },
				{ "left", "NOUN", "NNP", "NNS", "NNPS", "DTNN", "DTNNS",
						"DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT" },
				{ "left", "S" } }); // copied from SBAR rule -- look more
									// closely when there's time
		headRules.put("UCP", new String[][] { { "left" } });
		headRules.put("VP", new String[][] {
				{ "left", "VBD", "VBN", "VBP", "VBG", "DTVBG", "VN", "DTVN",
						"VP", "RB", "X", "VB" },
				{ "left", "IN" },
				{ "left", "NNP", "NOFUNC", "NOUN", "DTNN", "DTNNP", "DTNNPS",
						"DTNNS", "DTNOUN_QUANT", "NOUN_QUANT" } }); // exclude
																	// RP
																	// because
																	// we don't
																	// want
		...
		headRules.put("WHADVP", new String[][] { { "left", "WRB", "WP" },
				{ "right", "CC" }, { "left", "IN" } });
	...
	private static void loadRules() {
		...
		headRules.put("NP", new String[][] {
				{ "rightdis", "NN", "NNP", "NNPS", "NNS", "NX", "NML", "JJR" },
				{ "left", "NP", "PRP" },
				{ "rightdis", "$", "ADJP", "FW" },
				{ "right", "CD" },
				{ "rightdis", "JJ", "JJS", "QP", "DT", "WDT", "NML", "PRN",
						"RB", "RBR", "ADVP" }, { "left", "POS" }, });
		headRules.put("WHNP", new String[][] {
				{ "rightdis", "NN", "NNP", "NNPS", "NNS", "NX", "NML", "JJR",
						"WP" }, { "left", "WHNP", "NP" },
				{ "rightdis", "$", "ADJP", "PRN", "FW" }, { "right", "CD" },
				{ "rightdis", "JJ", "JJS", "RB", "QP" },
				{ "left", "WHPP", "WHADJP", "WP$", "WDT" } });
		// WHADJP
		headRules.put("WHADJP", new String[][] {
				{ "left", "ADJP", "JJ", "JJR" }, { "right", "RB" }, });
		headRules.put("WHADVP", new String[][] { { "rightdis", "WRB", "WHADVP",
				"RB", "JJ" }, });

		headRules.put("QP", new String[][] { { "right", "$", "NNS", "NN", "CD",
				"JJ", "PDT", "DT", "IN", "RB", "NCD", "QP", "JJR", "JJS" } });

		headRules.put("S", new String[][] {
				{ "left", "VP", "S", "FRAG", "SBAR", "ADJP", "UCP", "TO" },
				{ "right", "NP" } });

		headRules.put("SBAR", new String[][] { { "left", "S", "SQ", "SINV",
				"SBAR", "FRAG", "VP", "WHNP", "WHPP", "WHADVP", "WHADJP", "IN",
				"DT" } });

		headRules.put("SQ", new String[][] { { "left", "VP", "SQ", "ADJP",
				"VB", "VBZ", "VBD", "VBP", "MD", "AUX", "AUXG" } });

		headRules.put("UCP", new String[][] {});

		headRules.put("CONJP", new String[][] { { "right", "VB", "JJ", "RB",
				"IN", "CC" }, });

		headRules.put("FRAG", new String[][] { { "left", "IN" },
				{ "right", "RB" }, { "left", "NP" },
				{ "left", "ADJP", "ADVP", "FRAG", "S", "SBAR", "VP" }, });

		headRules.put("PP", new String[][] {
				{ "right", "IN", "TO", "VBG", "VBN", "RP", "FW", "JJ" },
				{ "left", "PP" } });

		headRules.put("PRN", new String[][] { { "left", "VP", "SQ", "S",
				"SINV", "SBAR", "NP", "ADJP", "PP", "ADVP", "INTJ", "WHNP",
				"NAC", "VBP", "JJ", "NN", "NNP" }, });
