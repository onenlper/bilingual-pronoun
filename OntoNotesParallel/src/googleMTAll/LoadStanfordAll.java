...
public class LoadStanfordAll {

	static String base = "/users/yzcchen/chen3/ijcnlp2013/googleMTALL/";
	...
	private static void outputCoNLL(int id) {
		if (!(new File(base + "/stanford/" + id + "." + targetLang + ".xml"))
				.exists()) {
			System.out
					.println("/users/yzcchen/chen3/ijcnlp2013/googleMTALL/eng_MT/raws/"
							+ id + ".chi");
			return;
		}
		...
		// }
		String filePath = allFiles.get(id);
		String docName = Util.getID(filePath);
		...
		for (StanfordSentence ss : sr.sentences) {
			...
			String parseTree = ss.parseTree.root.getTreeBankStyle(true)
					...
					.replace("(CC both)", "(DT both)");

			// if(parseTree.contains("(X")) {
			// Common.bangErrorPOS(parseTree);
			// System.out.println("/users/yzcchen/chen3/ijcnlp2013/googleMTALL/chi_MT/raws/"
			// + id + ".eng" + "\tX");
			// return;
			// }
			...
			for (int i = 0; i < ss.tokens.size(); i++) {
				...
				key = key.replace("(CC both)", "(DT both)");
