...
public class TrainCCMTAll {
	...
	// use all MT-parallel, but gold parallel if possible, v23
	//TODO
	static void v23() {
		engAlign = "/users/yzcchen/chen3/ijcnlp2013/googleMTALL/eng_MT/align/";
		chiAlign = "/users/yzcchen/chen3/ijcnlp2013/parallelMTMix/chi_MT/align/";
		engDoc = "engCoNLL.train.0";
		mtEngDoc = "MT.engCoNLL.train.0";
		chiDoc = "chiCoNLL.train.0";
		mtChiDoc = "MixMT.chiCoNLL.train.0";
	}
	
	//./train-nostack-joint-spe.sh 0 twoM
...
//		loadAllOrig_PartTrainParallel();
		...
		for(Span eng : engSpans) {
			if(eng.getSinglePronoun()) {
				Parallel.engAllPronouns++;
			}
		}
		...
		for(Span chi : chiSpans) {
			if(chi.getSinglePronoun()) {
				Parallel.chiAllPronouns++;
			}
		}
		...
		System.out.println("CHIPronouns: " + Parallel.chiAllPronouns);
		System.out.println("ENGPronouns: " + Parallel.engAllPronouns);
		
//		Common.outputHashSet(Span.mappedChiMs, "MixMTTrainMapChiMention");
	}
