...
import ims.coref.training.AbstractTrainingExampleExtractor;
...
public class TrainCC {
	...
	public static void main(String[] args) throws IOException {
		...
		// load giza output
		Span.CC = true;
		// DocumentMap.loadRealGizaAlignResult(util.Util.tokenBAAlignBaseGold
		// + "/align/");
		// DocumentMap.loadRealBAAlignResult(util.Util.tokenBAAlignBaseGold
		// + "/align_/");
		...
		if (engME.needsTraining()) {
			System.out.println("Training anaphoricy classifier: eng");
		...
		if (chiME.needsTraining()) {
			System.out.println("Training anaphoricy classifier: chi");
			chiME.train(chiReader);
		}
		...
		for (Document d : chiReader) {
			// need to extract spans
			chiSpans.addAll(chiME.extractMarkables(d));
			Chain[] cs = gsce.getGoldChains(d);
			AbstractTrainingExampleExtractor.pruneNested(cs);
			for (Chain c : cs) {
				if (c == null) {
					continue;
				}
				chiSpans.addAll(c.spans);
				chiGold += c.spans.size();
				chiGoldChain++;
			}
		}
		// read once
		...
		// int k = 0;
		// for(SortedDoc sd : chiSortedDocs) {
		// //TODO
		...
		// System.out.println("===========================================");
		// k = 0;
		// for(SortedDoc sd : engSortedDocs) {
		// //TODO
		// System.out.println((k++));
		// sd.tostring();
		// }
		...
		System.out.println("ENG GOLD Mention: " + engGold);
		System.out.println("CHI GOLD Mention : " + chiGold);

		System.out.println("ENG GOLD Chain: " + engGoldChain);
		System.out.println("CHI GOLD Chain: " + chiGoldChain);
