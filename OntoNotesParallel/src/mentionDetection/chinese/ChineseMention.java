...
//			}
	...
	private ArrayList<EntityMention> getChNPMention(CoNLLPart part) {
		ArrayList<EntityMention> npMentions = chCommon.getAllNounPhrase(part
				.getCoNLLSentences());
	...
	private void goldPruneChMentions(ArrayList<EntityMention> mentions,
			CoNLLPart part) {
		// TODO
		MentionDetect md = new GoldMentionTest();
	...
	private void pruneChMentions(ArrayList<EntityMention> mentions,
			CoNLLPart part) {
		for (EntityMention mention : mentions) {
		...
		for (int i = 0; i < mentions.size(); i++) {
			EntityMention mention = mentions.get(i);
			mention.position = chCommon.getPosition(mention,
					part.getCoNLLSentences());
			if (mention.ner.equalsIgnoreCase("QUANTITY")
					|| mention.ner.equalsIgnoreCase("CARDINAL")
					|| mention.ner.equalsIgnoreCase("PERCENT")
					|| mention.ner.equalsIgnoreCase("MONEY")) {
				removes.add(mention);
				continue;
			}

			if (mention.original.equalsIgnoreCase("我")
			...
			if (this.chCommon.getChDictionary().removeChars
			...
			if (mention.source.endsWith("的")
					|| (mention.source.endsWith("人")
							&& mention.start == mention.end && this.chCommon
								.getChDictionary().countries.contains(lastWord
							.substring(0, lastWord.length() - 1)))) {
				removes.add(mention);
				continue;
			}
			// ｑｕｏｔ
			if (this.chCommon.getChDictionary().removeWords
					.contains(mention.source)) {
				removes.add(mention);
				continue;
			}
		}
